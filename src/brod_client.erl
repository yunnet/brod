...
add_producer(ClientId, Topic, Partition) when is_atom(ClientId) ->
  io:format("ADD_PRODUCER: Adding new producer for client ~p, topic: ~p, partition: ~p~n", [ClientId, Topic, Partition]),
...
handle_cast({add_producer, Topic, Partition}, State) -> %%#state{producers_sup = ProducerSup} = State) ->
  ClientPid = self(),
